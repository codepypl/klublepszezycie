...
from models import db, EventSchedule, EventRegistration, User, Section, MenuItem, FAQ, BenefitItem, Testimonial, SocialLink
from app.services.email_service import EmailService
from app.blueprints.email_api import add_user_to_event_group
...
def get_database_data():
    ...
    try:
        ...
        # Debug about_section data
        if about_section:
            logging.info(f"About section found: enable_pillars={about_section.enable_pillars}, enable_floating_cards={about_section.enable_floating_cards}")
            logging.info(f"Pillars data: {about_section.pillars_data}")
            logging.info(f"Floating cards data: {about_section.floating_cards_data}")
        ...
        # FAQ data
        faqs_db = FAQ.query.filter_by(is_active=True).order_by(FAQ.order.asc()).all()
        faqs = [
            {'id': faq.id, 'question': faq.question, 'answer': faq.answer, 'order': faq.order}
            for faq in faqs_db
        ]
        ...
        # Active social links for footer
        active_social_links = SocialLink.query.filter_by(is_active=True).order_by(SocialLink.order).all()
        
        # If no CTA section in database, create minimal fallback
        if not cta_section:
            cta_section = {
        ...
        # If no FAQ section in database, create minimal fallback
        if not faq_section:
            faq_section = {
...
def privacy_policy():
    ...
    from models import LegalDocument, FooterSettings, SocialLink
