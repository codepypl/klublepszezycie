...
class User(UserMixin, db.Model):
    ...
    @property
    def is_admin(self):
        """Compatibility shim: treat admin as role-based"""
        return self.role == 'admin'

    @is_admin.setter
    def is_admin(self, value: bool):
        """Setting legacy is_admin updates the role accordingly"""
        self.role = 'admin' if value else (self.role if self.role != 'admin' else 'user')
...
# Old Registration model removed - functionality moved to EventRegistration
...
# SEO Settings removed - will be handled in configuration files
...
class EventSchedule(db.Model):
    ...
    is_published = db.Column(db.Boolean, default=False, index=True)  # Czy opublikowane na stronie
    ...
    def unarchive(self):
        """Odzarchiwizuje wydarzenie (przywraca do stanu aktywnego)"""
        self.is_archived = False
        # Don't automatically set is_active and is_published - let user decide
        return self
    



# Presentation Schedule removed - functionality moved to EventSchedule
...
# Footer Settings removed - will be handled in configuration files

# Legal Documents removed - will be handled as static files or configuration

# Blog Post Images removed - will be handled as JSON in BlogPost model
...
class LegalDocument(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    document_type = db.Column(db.String(50), nullable=False)  # 'privacy_policy', 'terms'
    title = db.Column(db.String(200), nullable=False)
    content = db.Column(db.Text)
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
